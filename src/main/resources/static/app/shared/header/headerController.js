angular
    ...
    .controller('main_headerCtrl', [
        ...
        '$location',
        'sweet',
        function ($timeout,$rootScope,$scope,$window,sessionService,$http,$cookies,Upload,mainService,Idle,$state,$location,sweet) {
        	  $scope.isLoggedIn = sessionService.isLoggedIn;
        	  mainService.withdomain('get','/user/service/ujson')
	  			.then(function(data){
	  				 $rootScope.user =  data;		
	  				 $scope.user=data;		
		  	  });	
	    	  ...
	    	  $scope.getUserAvatar = function($files){
	    		  if ($files.length > 0){
	    			  $scope.user.avatar_file = $files[0];
	    		  }
	    	  }
	    	  ...
	    	  $scope.submitChangePasswordReq = function(){
	          	  mainService.withdata("post","/changeUserPassword",$scope.p).then(function(data){
	          		  if (data == true){
	          			...
	          			sweet.show('Мэдэгдэл', 'Таны нууц үг амжилттай солигдлоо.', 'success');
	          		  }
	          	  });
	            }
	    	  
	    	   var changePasswordDialogModalObj = UIkit.modal("#changePasswordDialog");
	  		    $scope.changePasswordDialog=function(){
	  		    	UIkit.modal("#changePasswordDialog").show();
	  		    }
