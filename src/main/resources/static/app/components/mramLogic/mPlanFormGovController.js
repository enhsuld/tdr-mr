angular
    ...
    .controller('annualGOVFormCtrl', [
        ...
        function ($scope,$rootScope,$window, $state,$stateParams, mainService,fileUpload,sweet,org_data,sel_data,user_data,note_data,tz,ann,mineralData,rrq,mineType,concent,sgt) {        	
                    ...
                    mainService.withdata('put','/logic/submitComment', data)
			   			.then(function(data){	
			   				...
			   				if(data.comdata[0].step){
			   					...
			   					$state.go('restricted.pages.step'+stp);	
			   				}
			   				else{
			   					mainService.withdomain('get','/user/service/anstep/com.peace.users.model.mram.LnkPlanTransition/'+$stateParams.param+'/'+$scope.note_form.tabid)
					   			.then(function(data){
					   				if(data){
					   					sweet.show('Анхаар', 'Төлөвлөгөө эцсийн шийдвэрлүү шилжлээ. Эцсийн шийдвэр гаргаснаар жагсаалтнаас хасагдана', 'success');
					   				}
					   			})
				   				/*var aaa=0;
            ...
            $scope.setAsTodotgol = function () {
                sweet.show({
                    title: 'Баталгаажуулалт',
                    text: 'Та энэ тайлан / төлөвлөгөөг тодотгох уу?',
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#DD6B55',
                    confirmButtonText: 'Тийм',
                    ...
                    if (inputvalue) {
                        mainService.withdomain('POST','/setas/todotgol/'+$stateParams.param)
                            .then(function(){
                                sweet.show('Анхаар!', 'Амжилттай тодотголоо.', 'success');
                                $state.go('restricted.pages.todotgol');
                            });
                    }else{
