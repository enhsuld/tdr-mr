angular
    .module('altairApp')
    	.controller("annualPlanStep1CtrlGov",[
    	                          ...
    	                          'sweet',
    	                          'p_deposit',
	        function ($scope,$rootScope,$state,utils,mainService,sweet,p_deposit) {   
	  		    ...
	  		    $scope.loadOnCheck = function(item) {	
		  		  	if(item.reporttype==3){
	  		    		...
	  		    		if(item.lictype==1){
		   					...
		   					$state.go('restricted.pages.GovPlanFormA',{param:item.id, reqid:item.reqid, id:item.repstepid});
		   				}	  		    		 
	  		    	}
	  		    	else{
	  		    		if(item.lictype==1){
	            ...
	            $scope.PlanExploration = {
		                dataSource: {
		                    ...
		                    transport: {
		                    	read:  {
		                    		url: "/user/step/1/3/AnnualRegistrationPlan",
		                    		data: {"custom":"where reporttype=3 and xtype != 0", sort:[{"field":"id","dir":"desc"}]},
