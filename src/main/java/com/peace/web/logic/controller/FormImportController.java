...
public class FormImportController {
	...
	public String generateReport(HttpServletResponse response, HttpServletRequest req, @PathVariable String type) throws JsonProcessingException, SQLException {
		...
		if (loguser != null){
			...
			else if (type.equalsIgnoreCase("DATA_EXCEL_MINREP5")){
				List<Object[]> resultObj = (List<Object[]>) dao.getNativeSQLResult("SELECT A.LICENSEXB, A.LICTYPE, A.LPNAME, A.LP_REG, A.DIVISIONID, A.REPORTYEAR, A.REPORTTYPE, A.MINID, A.DEPOSITID, A.GROUPID, D.DATA1, D.DATA2, D.DATA3, D.DATA4, D.DATA5, D.DATA6, D.DATA7, D.DATA8, D.DATA9, D.DATA10, D.DATA11 FROM DATA_EXCEL_MINREP5 D LEFT JOIN ANNUAL_REGISTRATION A ON A.ID = D.PLANID WHERE A.REPSTATUSID = 1 AND A.DIVISIONID = "+loguser.getDivisionid()+" ORDER BY D.PLANID, D.ID", "list");
				String[] headers = {"LICENSEXB", "LICTYPE", "LPNAME", "LP_REG", "DIVISIONID", "REPORTYEAR", "REPORTTYPE", "MINID", "DEPOSITID", "GROUPID", "DATA1", "DATA2", "DATA3", "DATA4", "DATA5", "DATA6", "DATA7", "DATA8", "DATA9", "DATA10", "DATA11"};
				if (resultObj != null){
					for(Object[] o : resultObj){
						JSONObject obj = new JSONObject();
						for(int i=0;i<headers.length;i++){
			...
			else if (type.equalsIgnoreCase("DATA_EXCEL_MINREP6_1")){
				List<Object[]> resultObj = (List<Object[]>) dao.getNativeSQLResult("SELECT A.LICENSEXB, A.LICTYPE, A.LPNAME, A.LP_REG, A.DIVISIONID, A.REPORTYEAR, A.REPORTTYPE, A.MINID, A.DEPOSITID, A.GROUPID, D.DATA1, D.DATA2, D.DATA3, D.DATA4, D.DATA5, D.DATA6  FROM DATA_EXCEL_MINREP6_1 D LEFT JOIN ANNUAL_REGISTRATION A ON A.ID = D.PLANID WHERE A.REPSTATUSID = 1 AND A.DIVISIONID = "+loguser.getDivisionid()+" ORDER BY D.PLANID, D.ID", "list");
				String[] headers = {"LICENSEXB", "LICTYPE", "LPNAME", "LP_REG", "DIVISIONID", "REPORTYEAR", "REPORTTYPE", "MINID", "DEPOSITID", "GROUPID", "DATA1", "DATA2", "DATA3", "DATA4", "DATA5", "DATA6"};
				if (resultObj != null){
					for(Object[] o : resultObj){
						JSONObject obj = new JSONObject();
						for(int i=0;i<headers.length;i++){
